...
def on_message(client, userdata, msg):
    ...
    # ─── CSV Logging for Tableau ─────────────────────────────────────────────
    try:
        csv_file = 'overheat_detection_tableau.csv'
        new_file = not os.path.exists(csv_file) or os.path.getsize(csv_file) == 0
        with open(csv_file, 'a') as f:
            if new_file:
                f.write("timestamp,model_temperature,masked_temperature,is_overheat,is_undercool\n")
            f.write(f"{t},{room_temp:.2f},{measured:.2f},{measured >= OVERHEAT},{measured <= COLD_ALERT}\n")
    except Exception as e:
        console.error(f"[CSV Logger] Failed to write: {e}")
