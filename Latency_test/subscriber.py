...
# NEW: record test rate (1 or 10) for latency comparisons
RATE_HZ       = int(os.getenv('RATE_HZ', '1'))
...
def on_message(client, userdata, msg):
    ...
    t_pub_ms  = data.get('t_pub_ms')  # For latency calculation
    t         = datetime.fromisoformat(t_str.replace('Z','+00:00'))

    # Calculate latency
    if t_pub_ms is not None:
        now_ms  = int(time.time() * 1000)
        latency = now_ms - int(t_pub_ms)

        # Append to latency CSV (include test rate)
        write_header = not os.path.exists(LATENCY_CSV)
        with open(LATENCY_CSV, 'a', newline='') as f:
            w = csv.writer(f)
            if write_header:
                w.writerow(['timestamp', 'masked_temp', 'latency_ms', 'rate_hz'])
            w.writerow([t_str, measured, latency, RATE_HZ])

        console.info(f"[LAT] {t_str}  {latency} ms  (rate={RATE_HZ} Hz)")
    ...
    # Protected log: full details (incl. control effort)
...
console.info(f"[Subscriber] Starting HVAC loop…  (RATE_HZ={RATE_HZ})")
client.loop_forever()
